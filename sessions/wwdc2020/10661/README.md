---
id: "wwdc2020-10661"
event: "wwdc2020"
year: 2020
title: "What’s new with in-app purchase"
type: "Video"
url: "https://developer.apple.com/videos/play/wwdc2020/10661"
topics: ["App Store, Distribution & Marketing"]
platforms: ["iOS", "iPadOS", "macOS", "tvOS", "watchOS"]
hasTranscript: true
---

# What’s new with in-app purchase

**Event:** WWDC20 · **Topic:** App Store, Distribution & Marketing · **Platforms:** iOS, iPadOS, macOS, tvOS, watchOS · **Published:** 2020-06-24 · **Session:** [wwdc2020-10661](https://developer.apple.com/videos/play/wwdc2020/10661)

Create a great in-app purchase experience for your iPhone, iPad, Mac, and Apple Watch apps. Discover how to handle refunds, integrate new App Store server notifications, and find out how to use receipts and server notifications to manage subscriber status. We’ll also walk you through the latest updates in StoreKit, including in-app purchases on Apple Watch, Family Sharing, SKOverlay, SKAdNetwork, and more.

**Keywords:** `notifications`, `receipt`, `server`, `storekit`, `subscription`, `subscription business model`

## Transcript

[Read the transcript](transcript.md) · [Structured JSON](transcript.json)
(7,458 words)

## Documentation & Resources

- [Choosing the right functionality for your App Clip](https://developer.apple.com/documentation/AppClip/choosing-the-right-functionality-for-your-app-clip) _documentation_
  - Markdown (sosumi.ai): https://sosumi.ai/documentation/AppClip/choosing-the-right-functionality-for-your-app-clip
  - DocC JSON: https://developer.apple.com/tutorials/data/documentation/AppClip/choosing-the-right-functionality-for-your-app-clip.json
- [Handling Subscriptions Billing](https://developer.apple.com/documentation/StoreKit/handling-subscriptions-billing) _documentation_
  - Markdown (sosumi.ai): https://sosumi.ai/documentation/StoreKit/handling-subscriptions-billing
  - DocC JSON: https://developer.apple.com/tutorials/data/documentation/StoreKit/handling-subscriptions-billing.json
- [Handling refund notifications](https://developer.apple.com/documentation/StoreKit/handling-refund-notifications) _documentation_
  - Markdown (sosumi.ai): https://sosumi.ai/documentation/StoreKit/handling-refund-notifications
  - DocC JSON: https://developer.apple.com/tutorials/data/documentation/StoreKit/handling-refund-notifications.json
- [Validating Receipts with the App Store](https://developer.apple.com/documentation/storekit/original_api_for_in-app_purchase/validating_receipts_with_the_app_store) _documentation_
  - Markdown (sosumi.ai): https://sosumi.ai/documentation/storekit/original_api_for_in-app_purchase/validating_receipts_with_the_app_store
  - DocC JSON: https://developer.apple.com/tutorials/data/documentation/storekit/original_api_for_in-app_purchase/validating_receipts_with_the_app_store.json
- [App Store Server Notifications](https://developer.apple.com/documentation/appstoreservernotifications) _documentation_
  - Markdown (sosumi.ai): https://sosumi.ai/documentation/appstoreservernotifications
  - DocC JSON: https://developer.apple.com/tutorials/data/documentation/appstoreservernotifications.json
- [Setting up promotional offers](https://developer.apple.com/documentation/StoreKit/setting-up-promotional-offers) _documentation_
  - Markdown (sosumi.ai): https://sosumi.ai/documentation/StoreKit/setting-up-promotional-offers
  - DocC JSON: https://developer.apple.com/tutorials/data/documentation/StoreKit/setting-up-promotional-offers.json
- [In-App Purchase](https://developer.apple.com/documentation/StoreKit/in-app-purchase) _documentation_
  - Markdown (sosumi.ai): https://sosumi.ai/documentation/StoreKit/in-app-purchase
  - DocC JSON: https://developer.apple.com/tutorials/data/documentation/StoreKit/in-app-purchase.json
- [Enabling App Store Server Notifications](https://developer.apple.com/documentation/StoreKit/enabling-app-store-server-notifications) _documentation_
  - Markdown (sosumi.ai): https://sosumi.ai/documentation/StoreKit/enabling-app-store-server-notifications
  - DocC JSON: https://developer.apple.com/tutorials/data/documentation/StoreKit/enabling-app-store-server-notifications.json
- [Auto-renewable subscriptions overview](https://developer.apple.com/app-store/subscriptions/) _documentation_
  - Markdown (sosumi.ai): https://sosumi.ai/app-store/subscriptions/
- [Learn more about App Store Connect](https://help.apple.com/app-store-connect/) _guide_
- [StoreKit](https://developer.apple.com/documentation/StoreKit) _documentation_
  - Markdown (sosumi.ai): https://sosumi.ai/documentation/StoreKit
  - DocC JSON: https://developer.apple.com/tutorials/data/documentation/StoreKit.json

## Video

- HLS stream: https://devstreaming-cdn.apple.com/videos/wwdc/2020/10661/8/F8CB79D2-B53B-4C5A-A3B2-E298F3ABEE9A/master.m3u8

---

_Source: [Apple Inc.](https://developer.apple.com/videos/play/wwdc2020/10661) — developer.apple.com. Indexed for agent consumption._