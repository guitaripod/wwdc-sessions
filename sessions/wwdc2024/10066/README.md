---
id: "wwdc2024-10066"
event: "wwdc2024"
year: 2024
title: "Build immersive web experiences with WebXR"
type: "Video"
url: "https://developer.apple.com/videos/play/wwdc2024/10066"
topics: ["App Services", "Spatial Computing", "Safari & Web"]
platforms: ["visionOS"]
hasTranscript: true
---

# Build immersive web experiences with WebXR

**Event:** WWDC24 · **Topic:** Safari & Web · **Platforms:** visionOS · **Published:** 2024-06-12 · **Session:** [wwdc2024-10066](https://developer.apple.com/videos/play/wwdc2024/10066)

Discover how WebXR empowers you to add fully immersive experiences to your website in visionOS. Find out how to build WebXR experiences that take full advantage of the input capabilities of visionOS, and learn how you can use Simulator to test WebXR experiences on macOS.

## Transcript

[Read the transcript](transcript.md) · [Structured JSON](transcript.json)
(4,037 words)

## Documentation & Resources

- [Wonderland Engine](https://wonderlandengine.com) _samplecode_
- [A-Frame WebXR framework](https://aframe.io) _guide_
- [WebXR Chess Garden Demo](https://webkit.org/demos/webxr-chess/) _documentation_
- [WebXR Device API | W3C](https://www.w3.org/TR/webxr/) _documentation_
- [Forum: Safari & Web](https://developer.apple.com/forums/topics/safari-and-web?cid=vf-a-0010) _developerForum_
  - Markdown (sosumi.ai): https://sosumi.ai/forums/topics/safari-and-web?cid=vf-a-0010
- [WebKit.org – Report issues to the WebKit open-source project](https://bugs.webkit.org) _documentation_
- [PlayCanvas – webGL and WebXR library](https://playcanvas.com) _samplecode_
- [babylon.js – webGL and WebXR library](https://www.babylonjs.com) _samplecode_
- [three.js – webGL and WebXR library](https://threejs.org) _samplecode_
- [Adding a web development tool to Safari Web Inspector](https://developer.apple.com/documentation/SafariServices/adding-a-web-development-tool-to-safari-web-inspector) _documentation_
  - Markdown (sosumi.ai): https://sosumi.ai/documentation/SafariServices/adding-a-web-development-tool-to-safari-web-inspector
  - DocC JSON: https://developer.apple.com/tutorials/data/documentation/SafariServices/adding-a-web-development-tool-to-safari-web-inspector.json
- [Safari Release Notes](https://developer.apple.com/documentation/safari-release-notes) _documentation_
  - Markdown (sosumi.ai): https://sosumi.ai/documentation/safari-release-notes
  - DocC JSON: https://developer.apple.com/tutorials/data/documentation/safari-release-notes.json
- [WebKit Open Source Project](https://webkit.org) _guide_
- [Web Inspector Reference](https://webkit.org/web-inspector/) _documentation_

## Video

- HLS stream: https://devstreaming-cdn.apple.com/videos/wwdc/2024/10066/4/688EE2E3-325B-40BE-B36E-7067500DE40B/cmaf.m3u8
- Download: https://devstreaming-cdn.apple.com/videos/wwdc/2024/10066/4/688EE2E3-325B-40BE-B36E-7067500DE40B/cmaf-download.m3u8

---

_Source: [Apple Inc.](https://developer.apple.com/videos/play/wwdc2024/10066) — developer.apple.com. Indexed for agent consumption._